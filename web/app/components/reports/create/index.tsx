...
import { fetchReportTemplate } from '@/service/reports'
        ...
        <div className="bg-accent size-full flex justify-center items-center">
            <div className="border rounded-md p-8 py-10 w-1/2 bg-card">
                ...
                <div className="flex gap-2 mt-4">
                    <Button className="w-full" onClick={createDocx}>{t('report.newButton')}</Button>
                    <Button variant="secondary" disabled={loading}  className="w-full border-gray-200" onClick={importDocx}>
...
const useReport = (versionKey: string, onchange: () => void) => {
    ...
    useEffect(() => {
        fetchReportTemplate(versionKey).then(res => {
