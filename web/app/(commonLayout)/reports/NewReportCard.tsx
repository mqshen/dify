...
const CreateAppCard = ({ ref }: CreateAppCardProps) => {
  ...
  return (
    ...
    >
      <Link ref={ref} className='group flex grow cursor-pointer items-start p-4' href={`${basePath}/reports/create`}>
        <div className='flex items-center gap-3'>
          ...
          <div className='system-md-semibold text-text-secondary group-hover:text-text-accent'>{t('report.createDataset')}</div>
        </div>
      </Link>
      <div className='system-xs-regular p-4 pt-0 text-text-tertiary'>{t('report.createDatasetIntro')}</div>
      <Link className='group flex cursor-pointer items-center gap-1 rounded-b-xl border-t-[0.5px] border-divider-subtle p-4' href={`${basePath}/reports/connect`}>
        <div className='system-xs-medium text-text-tertiary group-hover:text-text-accent'>{t('report.connectDataset')}</div>
